...
function deadlineColor(deadline: string | null) {
  ...
  const days = Math.ceil(
    (new Date(`${deadline}T00:00:00`).getTime() -
      new Date(`${todayInKst()}T00:00:00`).getTime()) /
      86_400_000,
  );
...
export function CertificateApplyForm({
  ...
  const selectable = data.eligibleCourses.filter(
    (course) => !course.alreadyApplied,
  );
  ...
  const [checked, setChecked] = useState<string[]>(() => {
    ...
    return preset
      ? [preset.courseId]
      : selectable.slice(0, 1).map((course) => course.courseId);
  });
  ...
  const selectedCourses = data.eligibleCourses.filter((course) =>
    checked.includes(course.courseId),
  );
  const prepaidTotal = selectedCourses.reduce(
    (sum, course) => sum + course.prepaymentAmount,
    0,
  );
  const total = Math.max(
    0,
    data.issuanceCost * selectedCourses.length - prepaidTotal,
  );
    ...
    return (
      <div className="container">
        ...
        <div className="layout-side mt-5">
          <aside>
            <nav
              className="filter-group side-tabs side-tabs--2"
              aria-label="발급 메뉴"
            >
              <Link
              ...
              <Link className="side-nav__item" href="/certificate/status">
                발급 신청 현황
              </Link>
            </nav>
          </aside>
          ...
          <div>
            <div className="guide-box">
              ...
              <ul>
                <li>
                  수료 조건(진도율·시험 합격)을 충족한 과정만 신청할 수
                  있습니다.
                </li>
                <li>
                  수강중인 과정은 <Link href="/mylecture">나의 강의실</Link>에서
                  확인하세요.
                </li>
              </ul>
            </div>
        ...
        <div>
          ...
          <section className="issue-banner">
            <div className="issue-banner__txt">
              <p className="issue-banner__label">
                한국직업능력개발원 정식등록 민간자격증
              </p>
              <h2>
                자격증 발급 <em>상장형 · 카드형 동시 발급</em>
              </h2>
              <p>
                발급비 {won(data.issuanceCost)}에 협회 자격증 발급비용과
                택배비가 모두 포함됩니다. 자격증 취득 후 갱신이 필요 없는
                평생자격증입니다.
              </p>
            </div>
          ...
          <div className="card card--note mt-4">
            ...
            <ul
              className="guide-box"
              style={{ marginTop: 0, padding: 0, background: "none" }}
            >
              <li style={{ gridColumn: "auto" }}>
                배송 기간 : 매주 화요일 협회로 명단이 전달되며, 배송 완료는
                전달일로부터 1~2일 소요됩니다
              </li>
              <li style={{ gridColumn: "auto" }}>
                발급 금액 : {won(data.issuanceCost)} (협회 자격증 발급비용 ·
                택배비 포함 / 집체교육 진행 시 200,000원)
              </li>
              <li style={{ gridColumn: "auto" }}>
                본 교육원의 자격증은 한국직업능력개발원에 등록된 자격과정으로,
                지정기관 등록의 제공인력(교강사) 자격요건으로도 사용 가능합니다
              </li>
              <li style={{ gridColumn: "auto" }}>
                자격증 발급은 한국직업능력개발원 정식 자격등록번호를 부여하여
                한국직업능력교육협회 명의로 발급됩니다
              </li>
              <li style={{ gridColumn: "auto" }}>
                사진(선택)을 첨부하면 카드형 자격증에 넣어 드립니다. 셀카도
                가능하며, 없으면 사진 없이 발급됩니다
              </li>
            </ul>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="card mt-4">
              <h2 className="card__title" style={{ marginBottom: "6px" }}>
                발급할 자격증 선택
              </h2>
              <p
                style={{
              ...
              >
                합격하신 과정만 표시됩니다. 2개 이상 신청하실 경우 각각 선택해
                주세요.
              </p>
              ...
              <table className="issue-table">
                ...
                <thead>
                  <tr>
                    ...
                    <th className="col-pass" scope="col">
                      합격일
                    </th>
                  ...
                  {data.eligibleCourses.map((course) => (
                    ...
                    >
                      ...
                      <td
                        className="due"
                        style={{
                          color: deadlineColor(course.applyDeadline),
                          fontWeight: 700,
                        }}
                      >
                        {course.applyDeadline ? (
                          <>
                            <DateWithCentury date={course.applyDeadline} />
                            까지
                          </>
                        ) : (
                          "—"
                      ...
                      <td className="tabular fee" data-short="10만원">
                        {won(data.issuanceCost)}
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              <p className="notice mt-4">
                <span>
                  <b>
                    합격 후 14일이 지나면 해당 과목이 초기화되어 발급 신청이
                    불가합니다.
                  </b>
                  기한 내에 신청해 주세요.
                </span>
            ...
            <div className="card mt-3">
              ...
              >
                {["셀카 가능", "복장 자유", "자격증 효력과 무관"].map((tag) => (
                  <span
                    key={tag}
                    style={{
                      padding: "4px 10px",
              ...
              >
                ...
                >
                  {photoPreview ? (
                    ...
                    <span className="ph ph--cert" aria-hidden="true">
                      사진<small>선택</small>
                    </span>
                  )}
                </div>
                <div className="field" style={{ flex: 1, minWidth: "220px" }}>
                  <label htmlFor="photo">
                    사진 파일 <span className="hint">(JPG/PNG, 5MB 이하)</span>
                  </label>
                  <input
                    id="photo"
                    type="file"
                  ...
                  >
                    카드형 자격증에 넣을 사진입니다. 증명사진이 아니어도
                    괜찮아요 — 없으면 사진 없이 깔끔하게 발급됩니다.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="card mt-3">
              ...
              <div className="fee-summary">
                <p className="fee-summary__row">
                  자격증 발급비 <b>{won(data.issuanceCost)}</b> ×{" "}
                  <span>{selectedCourses.length}</span>건
                </p>
                {prepaidTotal > 0 ? (
                  <p className="fee-summary__row">
                    선납 반영 <b>-{won(prepaidTotal)}</b>
                  </p>
                ) : null}
                <p className="fee-summary__total">
                  최종 결제금액 <b>{won(total)}</b>
                </p>
              </div>
              ...
              <div className="form mt-4" style={{ maxWidth: "none" }}>
                <div className="field">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setPaymentMethod(
                        event.target.value as "card" | "bank_transfer",
                      )
                    }
                  >
              ...
              <p className="notice mt-3">
                <span>
                  무통장 입금 시 <b>신한은행 140-015-773620 (주)한평생그룹</b>{" "}
                  계좌로 입금해 주세요. 반드시 <b>수강자 본인 이름</b>으로
                  입금하셔야 하며, 입금자명과 수강자명이 다를 경우 확인 전화를
                  드립니다. 입금자명 옆에 생년월일을 함께 기재해 주시면 처리가
                  빠릅니다. (예: 홍길동0320)
                </span>
              </p>
            </div>
            ...
            <p className="agree mt-3" style={{ maxWidth: "none" }}>
              ...
              />
              <label htmlFor="agree">
                <b>[필수] 배송지 정보 수집 및 자격증 발급 진행에 동의합니다</b>
                <br />
                배송지 주소와 연락처를 반드시 확인해 주세요. 잘못된 정보로
                반송될 경우 재배송 비용은 착불로 배송됩니다.
              </label>
            </p>
