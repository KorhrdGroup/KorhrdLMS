import type { LectureListQuery } from "@/features/lectures/types/lecture.types";
...
): value is LectureListQuery["publish"] {
  return value === "published" || value === "unpublished";
}

export function parseLectureListQuery(
...
export function buildLectureListQueryString(
  params: Partial<LectureListQuery>,
  base?: LectureListQuery,
): string {
  const merged: LectureListQuery = {
...
export function buildLecturePageHref(page: number, query: LectureListQuery) {
  return `/admin/lectures${buildLectureListQueryString({ page }, query)}`;
}
