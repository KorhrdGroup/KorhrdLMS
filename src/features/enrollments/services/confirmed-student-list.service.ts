...
export async function getConfirmedStudentList(
  query: ConfirmedStudentListQuery,
): Promise<PaginatedResult<ConfirmedStudentListItem>> {
  const supabase = await createClient();
  const { from, to } = getPaginationRange(query.page, query.pageSize);

  let builder = supabase
    .from("enrollments")
    .select(CONFIRMED_STUDENT_LIST_SELECT, { count: "exact" })
    .eq("status", "confirmed")
    .is("deleted_at", null)
  ...
  return {
    data: (data ?? []) as ConfirmedStudentListItem[],
    total,
    page: query.page,
...
export async function getConfirmedStudentFilterOptions(): Promise<ConfirmedStudentFilterOptions> {
  const supabase = await createClient();
